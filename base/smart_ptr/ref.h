...
namespace rolling::base {
    ...
    template <typename T>
    struct CanEnableShareFromThis<
        T,
        std::void_t<decltype(std::declval<T>().ShareFromThis()),
                    std::is_same<SharedPtr<T>,
                                 decltype(std::declval<T>().ShareFromThis())>>>
        : std::true_type {};
    ...
    template <typename T> class SharedPtr {
      public:
        ...
        template <class T2,
                  std::enable_if_t<std::_SP_pointer_compatible<T2, T>::value,
                                   int> = 0>
    ...
    template <typename T> class WeakPtr {
      public:
        ...
        operator bool() {
            return block != nullptr && block->data != nullptr &&
                   block->IsAlive();
        }
